...
parser.add_argument(
    "--vocab", type=str, default="/content/latent-pockets/data/vocab.txt"
)
parser.add_argument("--atom_vocab", default=common_atom_vocab)
parser.add_argument("--save_dir", type=str, default="/content/latent-pockets/data/")
...
class JointVAE(nn.Module):
    def __init__(self):
        ...
        model_state, optimizer_state, total_step, beta = torch.load(
            "/content/latent-pockets/data/model.ckpt"
        )
    ...
    def forward(self, pocket, ligand, beta=0.1, perturb_z=True):
        ...
        root_vecs, kl_div = self.ligand_vae.rsample(
            root_vecs, self.ligand_vae.R_mean, self.ligand_vae.R_var, perturb_z
        )
    ...
    def predict(self, pocket, nsamples: int = 10):
        ...
        root_vecs, kl_div = self.ligand_vae.rsample(
            root_vecs, self.ligand_vae.R_mean, self.ligand_vae.R_var, perturb=True
        )
...
df = pd.read_csv("/content/latent-pockets/data/standardized_data_cleaned.csv")

base_fp = Path("/content/latent-pockets/data/structures/pockets_6A")
proteins = [
    base_fp / f"{chembl_id}_{pdb}_pocket.pdb"
    for chembl_id, pdb in zip(df["target_chembl_id"], df["pdb"])
]
