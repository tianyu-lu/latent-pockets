...
class HierVGNN(nn.Module):
    ...
    def translate(self, tensors, num_decode, enum_root, greedy=True):
        ...
        z_tree = torch.randn(batch_size, 1, self.latent_size).expand(
            -1, tree_vecs.size(1), -1
        )
        z_graph = torch.randn(batch_size, 1, self.latent_size).expand(
            -1, graph_vecs.size(1), -1
        )
...
class HierCondVGNN(HierVGNN):
    ...
    def translate(self, tensors, cond, num_decode, enum_root):
        ...
        z_tree = torch.randn(num_decode, 1, self.latent_size).expand(
            -1, tree_vecs.size(1), -1
        )
        z_graph = torch.randn(num_decode, 1, self.latent_size).expand(
            -1, graph_vecs.size(1), -1
        )
